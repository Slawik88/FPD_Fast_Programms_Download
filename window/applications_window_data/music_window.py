...
def create_music_window():  
    ...
    music_window.geometry(f"{1200}x{700}")  # Задание размеров окна
    music_window.title(f"{app_title} -- Musics")  # Задание заголовка окна
...
# Создаем 10 строк и столбцов с равными пропорциями
    ...
    conn = sqlite3.connect('data_app.db')
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM music_app')
