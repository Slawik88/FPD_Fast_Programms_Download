...
def create_programming_window():  
    ...
    programming_window.geometry(f"{1600}x{750}")  # Задание размеров окна
    programming_window.title(f"{app_title} -- Browsers")  # Задание заголовка окна

    # Создаем 10 строк и столбцов с равными пропорциями
    for i in range(10):
        programming_window.grid_rowconfigure(i, weight=1)
        programming_window.grid_columnconfigure(i, weight=1)
    ...
    def handle_search():
        ...
        print(search_text)
        for child in programming_scrollable_frame.winfo_children():
            child.destroy()
    ...
    def render_list(search_text=""):
        frame_search_bar = customtkinter.CTkFrame(master=programming_scrollable_frame)
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM programming_app WHERE app_name LIKE '%' || ? || '%'", (search_text,))
