...
def create_game_window():  
    game_window = customtkinter.CTkToplevel()  # Создание верхнего уровня окна
    game_window.geometry(f"{1600}x{750}")  # Задание размеров окна
    game_window.title(f"{app_title} -- Game")  # Задание заголовка окна
    ...
    game_scrollable_frame = customtkinter.CTkScrollableFrame(master=game_window, width=1200, height=1000, label_text="Browser_apps", orientation="vertical")
    game_scrollable_frame.grid(row=0, column=0, padx=20, pady=20)  # Размещение фрейма в окне
    ...
    def handle_search():
        ...
        print(search_text)
        for child in game_scrollable_frame.winfo_children():
            child.destroy()
    ...
    def render_list(search_text=""):
        frame_search_bar = customtkinter.CTkFrame(master=game_scrollable_frame)
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM game_app WHERE app_name LIKE '%' || ? || '%'", (search_text,))
        ...
        for row in cursor.fetchall():
            ...
            label_id_product = customtkinter.CTkLabel(master=game_scrollable_frame,
            ...
            label_image_icon_product = customtkinter.CTkLabel(master=game_scrollable_frame, text=None, image=image_init_icon_product)
            label_image_icon_product.grid(row=app_id + 1, column=1, padx=15, pady=15)


            # Создание элемента для отображения названия браузера
            label_name_product = customtkinter.CTkLabel(master=game_scrollable_frame, 
            ...
            label_name_company = customtkinter.CTkLabel(master=game_scrollable_frame, 
            ...
            label_size_product = customtkinter.CTkLabel(master=game_scrollable_frame, 
            ...
            button_link_site_product = customtkinter.CTkButton(master=game_scrollable_frame, 
            ...
            button_add_favorites = customtkinter.CTkButton(master=game_scrollable_frame,
                                                  text="Add Favorite",
                                                  font=("Monaco", 12, "bold"),
                                                  width=50,
                                                  height=20,
