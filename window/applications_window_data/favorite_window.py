...
def create_favorite_window():  
    favorite_window = customtkinter.CTkToplevel()  # Создание верхнего уровня окна
    favorite_window.geometry(f"{1600}x{750}")  # Задание размеров окна
    favorite_window.title(f"{app_title} -- Favorite")  # Задание заголовка окна

    # Создаем 10 строк и столбцов с равными пропорциями
    for i in range(10):
        favorite_window.grid_rowconfigure(i, weight=1)
        favorite_window.grid_columnconfigure(i, weight=1)

    # Создание прокручиваемого фрейма для браузеров
    favorite_scrollable_frame = customtkinter.CTkScrollableFrame(master=favorite_window, width=1200, height=1000, label_text="Browser_apps", orientation="vertical")
    favorite_scrollable_frame.grid(row=0, column=0, padx=20, pady=20)  # Размещение фрейма в окне
    ...
    def handle_search():
        ...
        print(search_text)
        for child in favorite_scrollable_frame.winfo_children():
            child.destroy()
    ...
    def render_list(search_text=""):
        frame_search_bar = customtkinter.CTkFrame(master=favorite_scrollable_frame)
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM favorite_app WHERE app_name LIKE '%' || ? || '%'", (search_text,))
        ...
        for row in cursor.fetchall():
            ...
            label_id_product = customtkinter.CTkLabel(master=favorite_scrollable_frame,
            ...
            label_image_icon_product = customtkinter.CTkLabel(master=favorite_scrollable_frame, text=None, image=image_init_icon_product)
            label_image_icon_product.grid(row=app_id + 1, column=1, padx=15, pady=15)


            # Создание элемента для отображения названия браузера
            label_name_product = customtkinter.CTkLabel(master=favorite_scrollable_frame, 
            ...
            label_name_company = customtkinter.CTkLabel(master=favorite_scrollable_frame, 
            ...
            label_size_product = customtkinter.CTkLabel(master=favorite_scrollable_frame, 
            ...
            button_link_site_product = customtkinter.CTkButton(master=favorite_scrollable_frame, 
            ...
            button_add_favorites = customtkinter.CTkButton(master=favorite_scrollable_frame,
                                                  text="Add Favorite",
                                                  font=("Monaco", 12, "bold"),
                                                  width=50,
                                                  height=20,
