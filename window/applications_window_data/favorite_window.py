...
def create_favorite_window():  
    ...
    favorite_window.geometry(f"{1200}x{700}")  # Задание размеров окна
    favorite_window.title(f"{app_title} -- Browsers")  # Задание заголовка окна
...
# Создаем 10 строк и столбцов с равными пропорциями
    ...
    conn = sqlite3.connect('data_app.db')
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM favorite_app')
